...
const VideoConverter: React.FC = () => {
  ...
  const convertVideo = useCallback(
    async (fileConversion: FileConversion): Promise<void> => {
      try {
        const ffmpeg = await initializeFFmpeg()

        // Update file status to converting
        setUploadedFiles((prev) =>
          prev.map((fc) => (fc.file === fileConversion.file ? { ...fc, status: 'converting', progress: 0 } : fc)),
        )
        ...
        if (outputFormat === 'gif') {
          ...
          args = ['-i', inputName, '-c', 'copy', '-movflags', '+faststart', outputName]
        }
        ...
        // Read output file
        const mimeType =
          outputFormat === 'gif' ? 'image/gif' : outputFormat === 'mp3' ? 'audio/mp3' : `video/${outputFormat}`
        ...
        const allCompleted = uploadedFiles.every(
          (fc) => fc.file === fileConversion.file || fc.status === 'completed' || fc.status === 'error',
        )
        ...
        setUploadedFiles((prev) =>
          prev.map((fc) => (fc.file === fileConversion.file ? { ...fc, status: 'error', error: errorMsg } : fc)),
        )
      }
    },
  ...
  const getStatusIcon = () => {
    switch (conversionState.phase) {
      case 'loading':
      case 'converting':
        return <Loader2 className='w-[20px] h-[20px] text-indigo-500 animate-spin' />
      case 'completed':
        return <CheckCircle className='w-[20px] h-[20px] text-green-600' />
      case 'error':
        return <AlertCircle className='w-[20px] h-[20px] text-red-500' />
      default:
        return <FileVideo className='w-[20px] h-[20px] text-indigo-500' />
    }
  }
        ...
        {uploadedFiles.length === 0 ? (
          <div
            ...
            <small className='text-sm text-gray-400'>
              Up to 5 files supported (.mkv, .avi, .mov, .mp4, .webm, .flv, .m4v)
            </small>
          </div>
        ) : (
                ...
                <div className='flex flex-col md:flex-row md:items-center md:justify-between gap-4'>
                  <div className='flex items-center gap-4'>
                    ...
                    <div>
                      ...
                      <p className='text-sm text-gray-500 m-0'>
                        {(fileConversion.file.size / (1024 * 1024)).toFixed(2)} MB
                      </p>
                    </div>
                  </div>
                  <div className='flex items-center gap-3'>
                    ...
                    {fileConversion.status === 'error' && (
                      <div
                        className={`flex items-center gap-2 text-red-500 ${fileConversion.error ? 'cursor-help' : ''}`}>
